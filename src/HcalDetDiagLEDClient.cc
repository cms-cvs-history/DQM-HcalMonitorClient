...
HcalDetDiagLEDClient::HcalDetDiagLEDClient(std::string myname, const edm::ParameterSet& ps)
{
  ...
  minerrorrate_ = ps.getUntrackedParameter<double>("DetDiagLED_minerrorrate",
						   ps.getUntrackedParameter<double>("minerrorrate",0.05));
  minevents_    = ps.getUntrackedParameter<int>("DetDiagLED_minevents",
...
{
 if (debug_>2) std::cout <<"\t\tHcalDetDiagLEDClient::calculateProblems()"<<std::endl;
  if(!dqmStore_) return;
...
bool HcalDetDiagLEDClient::hasErrors_Temp(void)
{
  if (!ProblemCells)
    {
      if (debug_>1) std::cout <<"<HcalDetDiagLEDClient::hasErrors_Temp>  ProblemCells histogram does not exist!"<<std::endl;
      return false;
    }
  int problemcount=0;
  int ieta=-9999;
...
bool HcalDetDiagLEDClient::hasWarnings_Temp(void){return false;}
...
} //void HcalDetDiagLEDClient::updateChannelStatus
